...
/* This one is the print function for each generation */
    ...
    /* Calculates diversity */
      ...
      for (i = 0; i < 8; i++) {
        fc = 0; fg = 0; ft = 0; fa = 0;
        for (j = 0; j < n; j++) {
          if(input[(int)VECTOR(pop)[j]][i] == 'C') {
            fc += 1;
          }
          else if(input[(int)VECTOR(pop)[j]][i] == 'G') {
            fg += 1;
          }
          else if(input[(int)VECTOR(pop)[j]][i] == 'T') {
            ft += 1;
          }
          else {
            fa += 1;
        ...
        fc /= (double)n; fg /= (double)n; ft /= (double)n; fa /= (double)n;
        if (fc > 0) {
          fc = (fc * log2(fc));
        }
        if (fg > 0) {
        ...
        VECTOR(aux3)[i] = - (fc + fg + ft + fa);
      }
      for (i = 0; i < 8; i++) {
        div += VECTOR(aux3)[i];
      }

      MATRIX(AUX, t + generations * w, 2) = div / 8.0;
